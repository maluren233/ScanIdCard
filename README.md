...
实现从相册选择或直接拍照获取身份证图片，自动识别身份证正反面，自动识别身份证中的关键信息。
...
- 从手机相册选择身份证照片进行识别
- 支持直接调用摄像头拍照识别身份证
- 自动判断并尝试识别身份证正反面（FRONT / BACK）
- 使用腾讯云 API v3 签名算法进行身份认证（非 SDK 实现）
- 通过 OkHttp 发送 HTTPS 请求
- 解析并处理 OCR 返回的 JSON 数据
- 在界面中展示姓名、身份证号、性别、民族、住址、签发机关、有效期等信息
- 支持 Android 模拟器与真机运行
...
2. 通过以下任意方式获取身份证图片：
   ...
   - 点击“拍照”，使用摄像头拍摄身份证  
3. 点击“开始识别”  
4. 应用默认先尝试按身份证正面（FRONT）进行识别  
5. 若正面识别结果为空或失败，自动尝试识别身份证背面（BACK）  
6. 接收并解析腾讯云 OCR 返回结果  
7. 在界面中展示识别出的身份证信息或错误提示  
